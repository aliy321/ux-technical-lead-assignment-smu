...
export type PlayerAction = "price_up" | "price_down" | "move_left" | "move_right";
...
export function resolveRound(state: GameState): RoundResult {
    ...
    for (const customer of state.customers) {
        const playerDistance = Math.abs(customer.positionKm - state.player.positionKm);
        const competitorDistance = Math.abs(customer.positionKm - state.competitor.positionKm);
        ...
        const nearest: Vendor = playerDistance < competitorDistance ? state.player : state.competitor;
...
function applyMovement(positionKm: number, direction: "left" | "right"): number {
